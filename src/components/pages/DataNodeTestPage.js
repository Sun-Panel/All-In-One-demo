...
export class DataNodeTestPage extends SunPanelPageElement {
  static properties = {
    _nodeKey: { state: true },
    _dataKey: { state: true },
  ...
  constructor() {
    ...
    this._dataKey = 'token';
    this._dataValue = '12345678';
    this._scope = 'user';
    this._nodeKey2 = 'userSettings';
    this._dataKeys = 'token,location';
    this._results = [];
    this._message = '';
  ...
  _handleScopeChange(newScope) {
    ...
    // 切换到应用级时，节点名改为 appConfig
    if (newScope === 'app') {
      this._nodeKey = 'appConfig';
      this._nodeKey2 = 'appConfig';
    } else {
      this._nodeKey = 'userSettings';
      this._nodeKey2 = 'userSettings';
    }
  ...
  async _setByKeysExample() {
    if (!this._nodeKey2) {
      this._showMessage('请输入节点名', true);
      return;
    }
    try {
      const api = this._getApi();
      ...
      await api.setByKeys(this._nodeKey2, exampleData);
      this._results = [...this._results, {
        action: `setByKeys(${this._nodeKey2}, ${JSON.stringify(exampleData)})`,
        result: '批量设置成功',
        time: new Date().toLocaleTimeString()
      }].slice(-10);
        ...
        <div class="two-columns">
          ...
          <div class="column">
            ...
            <div class="form-section">
              ...
              <button class="btn full" style="margin-top: 6px;" @click=${this._setByKeysExample}>批量储存示例数据</button>
            </div>
          </div>
        </div>
      ...
      <div class="info-panel">
        ...
        <div class="info-panel-content">
          ...
          <ul>
            ...
            <li>dataNode.user.getByKeys() - 批量获取用户数据</li>
            <li>dataNode.user.setByKey() - 设置用户数据节点</li>
            <li>dataNode.user.setByKeys() - 批量设置用户数据节点</li>
            <li>dataNode.user.delByKey() - 删除用户数据节点</li>
            <li>dataNode.app.getByKey() - 获取应用数据节点</li>
            <li>dataNode.app.getByKeys() - 批量获取应用数据</li>
            <li>dataNode.app.setByKey() - 设置应用数据节点</li>
            <li>dataNode.app.setByKeys() - 批量设置应用数据节点</li>
            <li>dataNode.app.delByKey() - 删除应用数据节点</li>
            <li>测试不同 scope (app/user) 配置</li>
          </ul>
        </div>
      </div>
    `;
